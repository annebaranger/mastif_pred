...
rbind(stemData_am$plot_shade,stemData_eu$plot_shade) |> 
  ...
  group_by(species) |> 
    # compute weighted quantiles of sgdd and wai, and correlation between wai and sgdd
    mutate(margin.temp=case_when(mat<=quantile(mat, probs=thresh,na.rm=TRUE)[[1]]~"cold",
                                 mat<quantile(mat, probs=0.5+thresh/2,na.rm=TRUE)[[1]]&
                                   mat>quantile(mat, probs=0.5-thresh/2,na.rm=TRUE)[[1]]~"midtemp",
                                 mat>=quantile(mat,probs=1-thresh,na.rm=TRUE)[[1]]~"hot"),
           margin.temp=factor(margin.temp,levels=c("midtemp","cold","hot")),
           margin.deficit=case_when(dh<quantile(dh,probs=thresh,na.rm=TRUE)[[1]]~"humid",
                                    dh<quantile(dh,probs=0.5+thresh/2,na.rm=TRUE)[[1]]&
                                      dh>quantile(dh,probs=0.5-thresh/2,na.rm=TRUE)[[1]]~"midhum",
  ...
  filter(margin.temp %in% c("hot","cold")) |>
  # group_by(biome,margin.temp) |> mutate(n_plot=n()) |> 
  # group_by(biome,margin.temp,shade_min) |> summarise(n_shade=100*n()/n_plot) |> distinct() |> 
  # ggplot(aes(shade_min,n_shade,fill=margin.temp))+
  # geom_col(position="dodge")+
  # facet_grid(~biome)
  ggplot()+
  geom_histogram(aes(shade_min,fill=margin.temp),stat="count",position="dodge")+
