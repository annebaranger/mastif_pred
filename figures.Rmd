...
```{r}
author_list<-read.csv2("author_rev/form_answers.csv") |> 
  rowwise() |> 
  mutate(authors_long=paste(paste(sapply(strsplit(First.Name," ")[[1]],
                                         function(x)paste0(substr(x,1,1),".")),
                                  collapse = " "),
                            Last.Name)) |> 
  ungroup()
author_first<-author_list |>
  filter(authors_long%in%c("M. Bogdziewicz","J. S. Clark","G. Kunstler","T. Cordonnier","A. O. Baranger","V. Journe")) |> 
  mutate(order=case_when(authors_long=="A. O. Baranger"~1,
                         authors_long=="J. S. Clark"~2,
                         authors_long=="T. Cordonnier"~3,
                         authors_long=="M. Bogdziewicz"~4,
                         authors_long=="V. Journe"~5,
                         authors_long=="G. Kunstler"~dim(author_list)[1]))

author_ordered<-author_list |> 
  filter(!authors_long%in%author_first$authors_long) |> 
  arrange(Last.Name) |> 
  mutate(order=row_number()+5) |> 
  rbind(author_first) |> arrange(order)

institution<-author_ordered |> 
  select(affiliation,order) |> 
  group_by(affiliation) |> summarise(order=min(order)) |> arrange(order) |> 
  mutate(order_instit=row_number()) |> select(-order)

institution |> 
  mutate(institution_name=paste0("$^{",order_instit,"}$ ",affiliation,"\\")) |> 
  pull(institution_name) |> 
  paste(collapse = "%x%")
author_ordered |> 
  left_join(institution,by="affiliation") |> 
  mutate(author_name=paste0(authors_long," $^{",order_instit,"}$")) |> 
  arrange(order) |> 
...
mastif_species <-rbind(tar_read(mastif.am,store="target_data")$df.tree, 
                  tar_read(mastif.eu,store="target_data")$df.tree) |> 
  select(plotID,lon,lat,species) |> 
  filter(species %in%c("fagusGrandifo","abiesProcera")) |> 
  mutate(species=case_when(species=="fagusGrandifo"~"Fagus grandifolia",
                           species=="abiesProcera"~"Abies procera")) |> 
  distinct()
mastif_all<-rbind(tar_read(mastif.am,store="target_data")$df.tree, 
                  tar_read(mastif.eu,store="target_data")$df.tree) |> 
  select(plotID,lon,lat,species) |> 
  filter(!species %in%c("fagusGrandifo","abiesProcera")) |> 
  distinct()
mastif_map<-ggplot()+
  geom_jitter(data=mastif_all,aes(lon,lat),fill="grey60",color="grey60",width = 0.01,height = 0.01,alpha=0.4)+
  geom_jitter(data=mastif_species,aes(lon,lat,color=species),size=2,width = 0.01,height = 0.01)+
...
```{r}
nfi_all<-rbind(tar_read(fecundity.eu_clim,store="target_nfi"),
               tar_read(fecundity.am_clim,store="target_nfi")) |> 
  select(plot,lon,lat,species) |> 
  filter(species!="fagusGrandifo") |>
  distinct() 
nfi_species<-rbind(tar_read(fecundity.eu_clim,store="target_nfi"),
                   tar_read(fecundity.am_clim,store="target_nfi")) |> 
  select(plot,lon,lat,species) |> 
  filter(species=="fagusGrandifo") |> 
  distinct() 
...
nfi_map<-ggplot()+
  geom_hex(data=nfi_all,aes(lon,lat),bins=100)+
  geom_point(data=nfi_species,aes(lon,lat),color="lightgreen",size=0.2,bins=100)+
  geom_sf(data=worldmap,fill=NA)+
  theme(legend.position ="bottom",# c(0.5,0.1),
        # legend.direction = "horizontal",
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank())+
  scale_fill_gradient(low="lightgrey",high="grey20",breaks=c(0,500,1500,3000))+
  ylim(c(25,70))+xlim(c(-145,35))+labs(fill="NFI plots count")
...
fec_map<-tar_read(fecundity.fit.25,store="target_fit_margin") |> 
  ...
  theme(legend.position="bottom",#c(0.8,0.3),
        ...
        axis.ticks = element_blank(),
        axis.title = element_blank())+
  scale_color_gradientn(colours=viridis(n=15),trans="log",breaks=c(0,1,100,10000))+
...
stemAcerRubrum<-stemData_eu$plot_shade |> 
...
cbind(stemAcerRubrum,
  ...
  theme(axis.text = element_blank(),
        ...
        legend.background = element_rect(fill="#e9ecef", 
                                  size=0.5,
                                  linetype="solid",
                                  color="#e9ecef"))->b
  # common_theme+
  # labs(title="B. PC1 of abiotic variables PCA")->b
...
cbind(stemAcerRubrum,
  ...
  theme(axis.text = element_blank(),
        ...
        legend.background = element_rect(fill="#e9ecef", 
                                  ...
                                  color="#e9ecef"))+
  # common_theme+
  labs(title="B. PC1 of biotic variables PCA")->d
...
fitbiomediscretedh<-tar_read(fitbiomediscrete_dh_25,store="target_fit_margin")
fecundity.fit<-tar_read(fecundity.fit.25,store="target_fit_margin")
...
species_n=fec_biome |>
  ...
  select(biome,margin.deficit,species) |> 
  unique() |> 
  group_by(biome,margin.deficit) |> 
  summarise(n=paste0("(",n(),")"))

signif<- fitbiomediscretedh |>
  filter(margin.deficit%in%c("arid","humid")) |> 
  group_by(biome,margin.deficit) |> 
  summarise(mean=mean(posterior),
            q05=quantile(posterior,prob=0.05),
...
text=fitbiomediscretedh |>
  filter(margin.deficit%in%c("arid","humid")) |> 
  group_by(biome,margin.deficit) |> summarise(med=median(posterior),ext=min(posterior)-0.1) |> 
  left_join(species_n,by=c("biome","margin.deficit")) |>
  left_join(signif) |>
  mutate(star=if_else(signif,"*",""),
         margin.deficit=case_when(margin.deficit=="arid"~"Arid",
                               margin.deficit=="humid"~"Humid"),
         biome=case_when(biome==4~"Temperate deciduous",
                         biome==5~"Temperate coniferous",
                         biome==6~"Boreal",
                         biome==12~"Dry"))
...
fitbiomediscretedh |>
  filter(margin.deficit%in%c("arid","humid")) |> 
  mutate(margin.deficit=case_when(margin.deficit=="arid"~"Arid",
                               margin.deficit=="humid"~"Humid"),
         biome=case_when(biome==4~"Temperate deciduous",
                         biome==5~"Temperate coniferous",
                         biome==6~"Boreal",
  ...
  geom_text(data=text,aes(label=n,x=med,y=biome),vjust=-0.5,size=3)+
  scale_fill_manual(values=c("turquoise3","tomato2","seagreen4","springgreen3"))+
  scale_color_manual(values=c("turquoise3","tomato2","seagreen4","springgreen3"),guide="none")+
  facet_wrap(~margin.deficit,nrow=2)+
  common_theme+
  theme(axis.text.y=element_blank(),
        axis.title.y=element_blank(),
        legend.position ="none",# c(0.13,0.13),
        legend.background = element_rect(fill = "white",
                                         size = 0.5),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())+
  labs(x="Relative log-fecundity \n(species as random effect)",
       title="A. Posterior of biome scale model",
       fill="Biome") ->boxp
```
...
fec_biome |>  
  ...
  scale_x_continuous(breaks=c(-0.4,-0.2,0,0.2,0.4,0.6))+
  facet_wrap(~biome,nrow=4)+
  labs(x="Pearson's correlation in species range \n (temperature / moisture deficit)",y="Count (species)",
       title="B. Correlation between temperature \nand moisture deficit")->b

cowplot::plot_grid(boxp,b,ncol=2,rel_widths =c(0.8,1))->p3

ggsave(p3, file="main_fig_3_margin_discrete.png",width=15,height = 15,dpi=600,units="cm")
```
...
species_n=fecundity.fit |> 
  ...
  group_by(zone,margin.temp) |> 
  summarise(n=paste0("(",n(),")")) |> 
  mutate(n=case_when(zone=="west"~"(20)",
                     zone=="east"~"(40)",
                     zone=="europe"~"(7)"))
...
```{r}
species.biome=tar_read(species.biome,store="target_fit")[,c(1,2)] |> 
  bind_rows(data.frame(species=c("abiesProcera","arbutusMenziesi","arbutusMenziesi",
                                 "fraxinusNigra","morusRubra","piceaGlauca","piceaGlauca",
                                 "quercusStellata","ulmusAlata"),
                       biome=c(5,5,12,4,4,5,6,4,4))) |> 
  mutate(biome=case_when(biome==8~4,
                         TRUE~biome),
         biome=case_when(biome==4~"Temp. dec.",
                         biome==5~"Temp. con.",
                         biome==6~"Boreal",
                         ...
                         biome==12~"Dry",
                         biome==13~"Desert"),
         biome=factor(biome,levels=c("Temp. dec.","Temp. con.","Boreal","Dry"))) |> 
  distinct()
data_env<-fecundity.fit |> 
  select(-biome) |> 
  filter(species!="pinusFlexilis") |> 
  filter(margin.temp%in%c("cold","hot"))|> 
  left_join(species.biome[,c("species","biome")]) |> 
  group_by(biome,species,margin.temp) |> 
  summarise(med.dh=median(dh),
            med.mat=quantile(mat,probs=0.5)) |> 
  group_by(margin.temp,biome) |> 
  mutate(max.mat=max(med.mat),
         min.mat=min(med.mat)) 
...
model_data <-fitspecies |>
  ...
  mutate(margin.temp=case_when(margin.temp=="hot"~"Hot",
                               margin.temp=="cold"~"Cold")) |> 
  group_by(biome,species,margin.temp,med.mat,max.mat,min.mat) |> 
  summarise(med.ISP=median(posterior),
            q05=quantile(posterior, prob=0.05)[[1]],
            q95=quantile(posterior, prob=0.95)[[1]]) |>
  mutate(signif=!(q05<0&q95>0)) |> ungroup() |> 
  ...
  filter(p_value<0.01) |>
  # filter(zone=="europe") |> 
  dplyr::select(term,estimate,biome,margin.temp,max.mat,min.mat)
...
fitspecies |>
  ...
  mutate(margin.temp=case_when(margin.temp=="hot"~"Hot",
                               margin.temp=="cold"~"Cold")) |> 
  group_by(biome,species,margin.temp,med.mat) |> 
  summarise(med.ISP=median(posterior),
            q05=quantile(posterior, prob=0.05)[[1]],
            q95=quantile(posterior, prob=0.95)[[1]]) |> 
  mutate(signif=!(q05<0&q95>0),
  ...
  ungroup() |> 
  ggplot()+
  geom_point(aes(med.mat,med.ISP,color=biome,shape=signif),size=0.5)+
  geom_linerange(aes(x=med.mat,ymin=q05,ymax=q95,color=biome),size=0.4)+
  geom_line(data = model_data,
            aes(mat.pred,pred.isp,color=biome),
            linewidth=0.4) +
  geom_hline(yintercept = 0,color="gray32")+
  geom_vline(xintercept = 0,color="gray32")+
  common_theme+
  ...
  scale_color_manual(values=c("springgreen3","seagreen4","turquoise3","tomato2"))+  # scale_color_manual(values=c("firebrick1","slateblue","firebrick4"))+
  scale_shape_manual(values=c(1,19),guide="none")+
  # scale_y_continuous(breaks = c(-2,-1,-0.5,0.5,1,2))+
  labs(title="By biome",
       y="Relative log-fecundity \n(by species)",
       x="Mean annual temperature of the margin",
       color="")+
  facet_wrap(~margin.temp,ncol = 2)->figbiome



cowplot::plot_grid(figzone,figbiome,ncol=1,labels = c("A","B"),vjust=1)->plot
# ggsave(plot,file="einplot.png",width=15,height = 11,units="cm",scale=1.1,dpi=600)
```
...
ggsave( p3,
        filename = "figure/supmat_fig_2_narrow.png",
        scale=1.1,
        dpi = 600,
        width = 15,
        height=8,
...
prop_test_results <- data_processed %>%
  ...
  summarize(n_hot = sum(n[margin.temp == "hot"], na.rm = TRUE),
            n_cold = sum(n[margin.temp == "cold"], na.rm = TRUE)) %>%
  rowwise() %>%
