...
fitspecies |>
  ...
  mutate(signif=!(q05<0&q95>0)) |> 
  ungroup() |> 
  mutate(pos_eff=med.ISP>0) |> group_by(zone) |> mutate(n_sp=n())
  group_by(zone,margin.temp,pos_eff) |> summarise(n_signif=sum(signif)/44)
...
```{r}
load("model/mod_select_narrow/biome_continous_25/anova_5.RData")
...
# biome<-
fitbiomecontinuous |> filter(biome==5,margin.temp=="hot") |> select(dh,fit,quant05,quant95) |> 
  pivot_longer(cols=c("fit","quant05","quant95"))


fecundity.fit |> group_by(biome)

```
## complementary analysis
```{r}
fecundity.fit<-tar_read(fecundity.fit.25,store="target_fit")
tar_load(species.biome,store="target_fit_narrow")
...
data_aridity |> 
  ...
  mutate(cor_dh_mat=case_when(pval_dh_mat<0.05~cor_dh_mat,
         ...
         cor_disp_dh=case_when(pval_disp_dh<0.05~cor_disp_dh,
                              TRUE~0)) |> 
  ggplot(aes(cor_dh_mat,cor_disp_dh,color=as.factor(biome)))+
  geom_point()
  # pivot_longer(cols=c("cor_dh_mat","cor_disp_dh")) |> 
  # ggplot(aes(value))+
  # geom_histogram()+geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  # facet_grid(biome~name)
  # 
  
for (b in unique(data_aridity$biome)){
  print(b)
  data_biome<-data_aridity |> 
    filter(biome==b)
  print(summary(lm(dISP~dh,data=data_biome)))
  print(cor(x=data_biome$dh,y=data_biome$mat))
}
fecundity.fit |> 
    select(- biome) |> 
      left_join(species.biome) |> 
