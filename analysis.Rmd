...
bardensity=fecundity.am_clim |> filter(BA!=0,species=="abiesBalsamea") |> 
  summarise(lat=mean(lat),
            lon=mean(lon))
barycenter= fecundity.am_clim |> filter(BA!=0,species=="abiesBalsamea") |> 
  mutate(dh=map-pet) |> 
...
fecundity.am_clim %>%
  filter(BA != 0, species == "abiesBalsamea") %>%
  mutate(dh=map-pet) |> 
  mutate(lISP = log(ISP / mean(ISP))) %>%
  ggplot() +
  geom_sf(data=ecoregions,aes(fill=ECO_NUM),alpha=0.2)+
  scale_fill_gradientn(colours = wes_palette(type="continuous",name="GrandBudapest1"),
                       guide="none")+
  ...
  scale_colour_manual(values= c( "#9467bd","#8c564b","#2ca01c","#1f77b4","#d62728","#ff7f0e"))+
...
bardensity=fecundity.eu_clim |> filter(BA!=0,species=="abiesAlba") |> 
  summarise(lat=mean(lat),
            lon=mean(lon))
barycenter= fecundity.eu_clim |> filter(BA!=0,species=="abiesAlba") |> 
  mutate(dh=map-pet) |> 
  select(lat,lon,tempSite,dh,BA,cec30,slope,aspect) |> 
  pivot_longer(cols=c(-"lat",-"lon")) |> 
  group_by(name) |> 
...
fecundity.eu_clim |> filter(BA!=0,species=="pinusSylvestr") |> 
  ...
  scale_fill_gradientn(colours = wes_palette(type="continuous",name="GrandBudapest1"),
                       guide="none")+
  geom_point(aes(x = lon, y = lat, color = tempSite),
             size=0.2,
             alpha = 0.4) +
  scale_color_gradientn(colours = wes_palette(type="continuous",name="Zissou1")) +
  new_scale_color() +  
