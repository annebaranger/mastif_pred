...
data.pca=fecundity.am_clim |> filter(BA!=0,species=="acerRubrum") |> 
  mutate(dh=map-pet) |> 
  select(lat,lon,tempSite,dh,BA,cec30,slope,aspect,ISP) |> 
  drop_na()
pca=prcomp(data.pca[,c("tempSite","dh","BA","cec30","slope","aspect")],scale=TRUE)
...
cbind(data.pca,
  ...
  pivot_longer(cols=c("lISP","PC1","PC2")) |> 
  group_by(name) |> 
