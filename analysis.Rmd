...
```{r}
load("model/biome_continous_25/anova_4.RData")
as.data.frame(fit) |> select(c("beta[4]","beta[6]")) |>
  mutate(slope=`beta[4]`+`beta[6]`) |> summarise(mean=mean(slope),q05=quantile(slope,probs = 0.05),q95=quantile(slope,probs = 0.95))
...
# load("data/america/inventory/pet.rdata")
# load("data/america/inventory/prec.rdata")
load("data/america/inventory/def.rdata") # moisture deficit
mean.def=apply(def, MARGIN = 1,mean) |> as.data.frame() |> tibble::rownames_to_column()
colnames(mean.def)=c("plot","moisture_deficit")

load("data/america/inventory/temp.rdata") # temperature
mean.temp=apply(temp, MARGIN = 1,mean) |> as.data.frame() |> tibble::rownames_to_column()
colnames(mean.temp)=c("plot","temperature")
rm(temp,def)

load("data/america/inventory/plotData.rdata") # moisture deficit
allPlots<-plotData |> 
  left_join(mean.def) |> 
  left_join(mean.temp)

load("data/america/inventory/stemData.rdata")
load("data/america/inventory/ISP.rdata")
ISP=as.data.frame(ISP) |> 
  tibble::rownames_to_column(var="plot") |> 
  select(plot,acerRubrum) |> 
  filter(acerRubrum!=0)
colnames(ISP)=c("plot","ISP")
stemAcerRubrum<-stemData |> 
  filter(species=="acerRubrum") |> 
  group_by(plot) |> 
  summarize(mean.diam=mean(diam),
            mean.shade=mean(shade)) |> 
  left_join(allPlots,by="plot") |> 
  left_join(ISP,by="plot") |> 
  select(plot,lat,lon,mean.diam,mean.shade,moisture_deficit,temperature,cec30,slope,aspect,ISP) |> 
  rename(Temperature=temperature,
         `Moisture deficit`=moisture_deficit,
         `Mean diameter`=mean.diam,
         `Mean shade index`=mean.shade,
         CEC=cec30,
         Aspect=aspect,
         Slope=slope) |> 
  drop_na()
pca=prcomp(stemAcerRubrum[,c("Temperature","Moisture deficit","Mean diameter","Mean shade index","CEC","Slope","Aspect")],scale=TRUE)
...
cbind(stemAcerRubrum,
  ...
  # mutate(lfec = scale(log(fecGmMu/ mean(fecGmMu)))) %>%
  rename(`B. Mean Fecundity`=lISP,
         `C. PCA axis 1`=PC1,
         `D. PCA axis 2`=PC2) |> 
  pivot_longer(cols=c("B. Mean Fecundity","C. PCA axis 1","D. PCA axis 2")) |> 
  group_by(name) |> 
