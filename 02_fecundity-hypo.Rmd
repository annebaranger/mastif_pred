...
## LMM with interactions between margins and contiuous var
...
sp.zone |> 
  ...
  ggplot(aes(mat,log(dISP),color=margin.deficit))+
  geom_point(aes(mat,log(dISP),color=margin.deficit))+
  geom_boxplot(aes(mat,y=-10,color=margin.deficit),position=position_dodge(width=5),outlier.colour = NA)+
  geom_smooth()+
  facet_wrap(~species)->plot
sp.zone |> 
  filter(!is.na(margin.temp)) |> 
  ggplot(aes(dh,log(ISP),color=margin.temp))+
  geom_point()+
  geom_smooth()+
  facet_wrap(~species)->plot
...
sp.mod=sp.zone |> 
  filter(!is.na(margin.temp)) |> 
  mutate(dh2=scale(dh^2),
         dh=scale(dh))
deficit.ancova=lme(log(ISP)~margin.temp*dh+margin.temp*dh2,
                   random=~1|species,
                   data=sp.mod)
summary(deficit.ancova)
...
data_zone=list(N=dim(sp.mod)[1],
                 ...
                 X=X,
                 NULL)
fit.zone.2 <- stan(file = "lmm_ancova.stan",
                 data=data_zone,
                 iter=1000,
                 chains=3,
                 core=3,
...
speciesranef=as.data.frame(summary(fit.zone.2)$summary) |> 
...
sp.mod |> 
  ...
  # summarise(n=n()) |> 
  # ungroup() |> 
  ggplot(aes(margin.temp,fill=species))+
  geom_histogram(stat = "count")+
  facet_wrap(~margin.temp)+
  scale_fill_manual(values=viridis(n=13))
```
